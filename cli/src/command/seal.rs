...
pub fn run(
    ...
    if args.object_auth.auth.is_some() {
    ...
    let sealed_obj_auth = args.object_auth.auth.as_deref().unwrap_or("").as_bytes();
    ...
    let sessions = get_auth_sessions(&cmd, &handles, io.session, args.parent_auth.auth.as_deref())?;

    let (resp, _) = chip.execute(&cmd, Some(&handles), &sessions)?;

    let create_resp = resp
        .Create()
        .map_err(|e| TpmError::Execution(format!("unexpected response type for Create: {e:?}")))?;
